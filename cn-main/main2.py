...
def run_git_download():
 ...
 if params["animatediff"]=="True":
  subprocess.run(f'git clone https://github.com/guoyww/animatediff {params["dir"]}/tmp/sd-webui-animatediff', shell=True)
 end_time=time.time()
...
def run_aria2c_download():
 ...
 if params["animatediff"]=="True":
  subprocess.run(f'aria2c --console-log-level=error -c -x 16 -s 16 -k 1M https://huggingface.co/guoyww/animatediff/resolve/main/mm_sd_v15_v2.ckpt -d {params["dir"]}/tmp/sd-webui-animatediff/model -o mm_sd_v15_v2.ckpt',shell=True)
 end_time=time.time()
